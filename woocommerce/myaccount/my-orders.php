...
<?php echo do_shortcode('[get_order_ids_by_email]'); ?>
</div>
...
$my_orders_columns = apply_filters(
	...
	array(
		'order-number'  => esc_html__( 'Purchase no.', 'woocommerce' ),
		'order-date'    => esc_html__( 'Date', 'woocommerce' ),
		'order-status'  => esc_html__( 'Status', 'woocommerce' ),
		'order-total'   => esc_html__( 'Amount', 'woocommerce' ),
		'order-actions' => '&nbsp;',
	)
);
					...
					<?php foreach ( $my_orders_columns as $column_id => $column_name ) : ?>
						<td class="<?php echo esc_attr( $column_id ); ?>" data-title="<?php echo esc_attr( $column_name ); ?>">
							...
							<?php elseif ( 'order-actions' === $column_id ) : ?>
								...
								if ( ! empty( $actions ) ) {
										echo '<a href="' . esc_url( $action[0]['url'] ) . '" class="button ' . sanitize_html_class( $key ) . '">' . esc_html( $action[0]['name'] ) . '</a>';
